...
# Word2Vec
def word_to_vector(x_words,train_x,test_x,dic_path):
    dic = corpora.Dictionary(x_words[:][:])  # each words to dic
    # 保存字典到文本文件
    ...
    for lines in test_x[:]:
        ...
        for word in lines:
            # 查找每个单词在字典中的编码
            line_vec.append(dic_set[word])
        values2.append(line_vec)
        line_vec = []
    return values1, values2,max_len
...
train_x = load_dataset('./data/train_magic.in')
test_x = load_dataset('./data/test_magic.in')
train_y = load_dataset('./data/train_magic.out')
test_y = load_dataset('./data/test_magic.out')

inn = train_x+test_x
out = train_y+test_y

x1,x2,lenx = word_to_vector(inn,train_x,test_x,'./save/dict_file_in.txt')
save_word_vec('./save/train_in_vec.txt',x1)
save_word_vec('./save/test_in_vec.txt',x2)
print(lenx)  # 141
y1,y2,leny = word_to_vector(out,train_y,test_y,'./save/dict_file_out.txt')
save_word_vec('./save/train_out_vec.txt',y1)
save_word_vec('./save/test_out_vec.txt',y2)
print(leny)  # 290